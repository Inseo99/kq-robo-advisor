...
def get_latest_fin_asof(fin_df, ticker, asof=None):
    """tickerì˜ ìµœê·¼ 4ë¶„ê¸° í‰ê·  ì¬ë¬´ì§€í‘œ (Point-in-Time).

    `date`ëŠ” íšŒê³„ê¸°ê°„ ì¢…ë£Œì¼ì´ë¯€ë¡œ ê·¸ëŒ€ë¡œ ì“°ë©´ ë¯¸ë˜ ì •ë³´ ëˆ„ìˆ˜ê°€ ìƒê¸´ë‹¤.
    src/kq_tool/data/financial_pit.pyì˜ ê³µì‹œ ì§€ì—° ê·œì¹™(Q1~Q3 +45ì¼,
    Q4 +90ì¼)ì„ ì ìš©í•´ asof ì‹œì ì— ê´€ì¸¡ ê°€ëŠ¥í•œ í–‰ë§Œ ì‚¬ìš©í•œë‹¤.
    """
    if fin_df is None:
        return {}
    try:
