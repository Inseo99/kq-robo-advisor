...
def normalize_financial_frame(fin_df: pd.DataFrame | None) -> pd.DataFrame:
    ...
    if "ticker" not in frame.columns and "level_0" in frame.columns:
        frame = frame.rename(columns={"level_0": "ticker"})
    if "아이템명" not in frame.columns and "level_1" in frame.columns:
        frame = frame.rename(columns={"level_1": "아이템명"})
