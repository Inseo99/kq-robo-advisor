...
def run_strategy(strat: dict, panels: dict) -> pd.DataFrame:
    ...
    tidx = adj_close.index
    eval_dates = build_eval_dates(cadence, tidx)
    # 12개월 모멘텀 워밍업: t-12M 가격이 존재하는 첫 eval부터
    eval_dates = [pd.Timestamp(d) for d in eval_dates]

    gate, _detail = S.build_gate(cadence, variant, eval_dates, kodex_daily, vix, credit, sp500)
    cash_period = C.CASH_MONTHLY if cadence == "M" else C.CASH_WEEKLY

    # 각 eval -> exec(익영업일)
    execs = []
    for d in eval_dates:
        ex = next_trading_day(tidx, d)
        execs.append(ex)
