"""12개 전략 결과를 엑셀 워크북(xlsx)으로 정리.

시트 구성:
  1) 요약        - 전략 12종 + 벤치마크 핵심지표(ALL_OOS) 한 눈에 비교
  2) 백테스트_조건 - 유니버스/비용/현금/위험회피(t1,t2)/폴드 등 실행 조건
  3~14) 전략별 시트 - 개요 + 현금비중 + 폴드별(IS/OOS) 성과, 매 구간 벤치마크(KODEX200) 병기
...
VARIANT_DESC = {
    ...
    "s3": "t2: KODEX200 종가 > 10M(43주) 이동평균 -> 100% 보유 / 아니면 전액 현금",
}
CADENCE_DESC = {"M": "월간(월말 평가 -> 익월 첫 거래일 진입)", "W": "주간(금요일 평가 -> 익영업일 진입)"}
...
def bench_row(mdf, cadence, fold, segment):
    return strat_row(mdf, BENCH, cadence, fold, segment)


# ── 시트 1: 요약 ─────────────────────────────────────────────────────────
def build_summary_sheet(wb, mdf, period_records):
    ws = wb.create_sheet("요약")
    ws["A1"] = "backtest-ksj 전략 12종 성과 요약 (전체 OOS: 2019-01 ~ 2026-05)"
    ...
    for strat in C.STRATEGIES:
        ...
        full = strat_row(mdf, code, cad, "FULL", "ALL")
        pr = period_records[code]
        cw_oos = cash_weight(pr, "2019-01-01", "2026-05-31")
        cw_full = cash_weight(pr)
        overview = f"{SELECTION_DESC[sel]} | {VARIANT_DESC[var]}"
        vals = [
            code, "월간" if cad == "M" else "주간",
            "모멘텀20" if sel == "mom20" else "KODEX200",
    ...
    # 벤치마크 비교행
    for cad in ("M", "W"):
        brow = bench_row(mdf, cad, "ALL_OOS", "OOS")
        vals = [f"069500({'월간' if cad=='M' else '주간' } 환산)", "-", "-", "-", "벤치마크: KODEX200 매수후보유",
    ...
    ws.freeze_panes = "A5"
    _autofit(ws, [14, 8, 10, 9, 60, 9, 9, 9, 9, 9, 12, 12, 14, 12])
...
def build_conditions_sheet(wb):
    ...
    rows = [
        ...
        ("", ""),
        ("t2 추세추종 (s3)", VARIANT_DESC["s3"]),
        ("t2 - 이동평균", f"{C.T2_MA_M}개월({C.T2_MA_W}주) KODEX200 종가 이동평균"),
        ("", ""),
...
def build_strategy_sheet(wb, mdf, strat, pr):
    ...
    ws["A2"] = (f"캐던스: {CADENCE_DESC[strat['cadence']]}   |   "
                f"종목선정: {SELECTION_DESC[strat['selection']]}")
    ws["A3"] = f"위험회피: {VARIANT_DESC[strat['variant']]}"
    ...
    headers = ["구간", "세그먼트", "기간", "CAGR", "MDD", "Sharpe", "Calmar",
               ...
               "벤치 CAGR", "벤치 MDD", "벤치 Sharpe", "벤치 Calmar", "CAGR 초과분(전략-벤치)"]
    header_row = 7
    for j, h in enumerate(headers, start=1):
        ws.cell(row=header_row, column=j, value=h)
    _style_header_row(ws, header_row, len(headers))
    ...
    for fold, seg in FOLD_SEG_ORDER:
        ...
        brow = bench_row(mdf, cad, fold, seg)
        if row is None:
            continue
